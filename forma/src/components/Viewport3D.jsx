...
export function Viewport3D({ pieces, selectedPiece, onSelectPiece }) {
  ...
  const project3D = useCallback((x, y, z, cx, cy, h) => {
    ...
    const scale = focalPx / (CAMERA.viewerDistance + pz) * zoom;
  ...
  const drawFn = useCallback((ctx, w, h) => {
    ...
    for (let i = -200; i <= 200; i += 40) {
      const a = project3D(i, 100, -200, cx, cy, h), b = project3D(i, 100, 300, cx, cy, h);
      ctx.beginPath(); ctx.moveTo(a.sx, a.sy); ctx.lineTo(b.sx, b.sy); ctx.stroke();
      const c = project3D(-200, 100, i + 100, cx, cy, h), d = project3D(200, 100, i + 100, cx, cy, h);
      ctx.beginPath(); ctx.moveTo(c.sx, c.sy); ctx.lineTo(d.sx, d.sy); ctx.stroke();
    }

    // Axes
    const origin = project3D(0, 0, 0, cx, cy, h);
    [[project3D(80,0,0,cx,cy,h), COLORS.red, "X"], [project3D(0,-80,0,cx,cy,h), COLORS.green, "Y"], [project3D(0,0,80,cx,cy,h), COLORS.blue, "Z"]].forEach(([end, col, label]) => {
    ...
    const sorted = [...pieces].map((p, i) => ({ ...p, idx: i })).sort((a, b) => {
      return project3D(b.x, b.y, b.z, cx, cy, h).depth - project3D(a.x, a.y, a.z, cx, cy, h).depth;
    });
    ...
    sorted.forEach((piece) => {
      ...
      const hangL = project3D(attach.left.x, frameY, attach.left.z, cx, cy, h);
      const hangR = project3D(attach.right.x, frameY, attach.right.z, cx, cy, h);
      const attachL = project3D(attach.left.x, attach.left.y, attach.left.z, cx, cy, h);
      const attachR = project3D(attach.right.x, attach.right.y, attach.right.z, cx, cy, h);
      ...
      curve.forEach((p, j) => {
        ...
        const projected = project3D(worldX, worldY, worldZ, cx, cy, h);
      ...
      for (let j = 0; j < curve.length; j += edgeStep) {
        ...
        const lx = p.x * piece.scale * SHAPE_SCALE;
        const ly = p.y * piece.scale * SHAPE_SCALE;
        const frontP = project3D(piece.x + lx * cosT, piece.y + ly, piece.z + lx * sinT, cx, cy, h);
        const backP = project3D(piece.x + lx * cosT + nxW, piece.y + ly, piece.z + lx * sinT + nzW, cx, cy, h);
      ...
      const labelPos = project3D(piece.x, piece.y, piece.z, cx, cy, h);
    ...
    ctx.beginPath();
    fc.forEach((c, i) => { const p = project3D(c[0],c[1],c[2],cx,cy,h); i===0?ctx.moveTo(p.sx,p.sy):ctx.lineTo(p.sx,p.sy); });
    ...
    const frameLabelPos = project3D(fcx + half, frameY, fcz - half, cx, cy, h);
  ...
  }, [pieces, selectedPiece, zoom, project3D]);
  ...
  const handleMouseUp = (e) => {
    ...
    if (!dragMoved.current) {
      ...
      for (let i = 0; i < pieces.length; i++) {
        const p = project3D(pieces[i].x, pieces[i].y, pieces[i].z, cx, cy, rect.height);
